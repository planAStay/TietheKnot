    ...
    <div className="bg-gradient-to-b from-rose-50 via-white to-white">
      ...
      <section className="container grid gap-10 py-14 lg:grid-cols-2 lg:items-center lg:py-20">
        <div className="space-y-4">
          <p className="text-sm uppercase tracking-wide text-rose-700">TieTheKnot</p>
          <Heading level={1} className="text-4xl font-semibold leading-tight text-zinc-900 sm:text-5xl">
        ...
        <div className="relative rounded-3xl bg-white p-8 shadow-[0_18px_60px_-24px_rgba(16,24,40,0.18)] ring-1 ring-rose-100">
          <div className="absolute -right-6 -top-6 h-24 w-24 rounded-full bg-rose-100 blur-3xl" aria-hidden />
          <div className="absolute -left-6 -bottom-8 h-24 w-24 rounded-full bg-indigo-100 blur-3xl" aria-hidden />
          <div className="space-y-4">
            <Heading level={3} className="text-2xl font-semibold text-zinc-900">
      ...
      <section className="container py-12 sm:py-16">
        <div className="flex flex-wrap items-center justify-between gap-4">
          <div>
            <p className="text-sm uppercase text-rose-700">Find vendors for every vibe</p>
            <Heading level={2} className="text-3xl font-semibold text-zinc-900">
        ...
        <div className="mt-8 grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
          {categories.map((cat) => (
            <div
              key={cat.id}
              className="group relative overflow-hidden rounded-2xl border border-rose-100 bg-white p-5 shadow-sm transition hover:-translate-y-1 hover:shadow-lg"
            >
              <div className="absolute -right-8 -top-10 h-24 w-24 rounded-full bg-rose-50 blur-2xl" aria-hidden />
              <Text className="text-xs uppercase text-rose-600">{cat.subcategories.length} options</Text>
              <Heading level={3} className="mt-1 text-xl font-semibold text-zinc-900">
                {cat.name}
              </Heading>
              <Text className="mt-1 text-sm text-zinc-600">{cat.description}</Text>
              <div className="mt-3 flex flex-wrap gap-2 text-xs text-zinc-500">
                {cat.subcategories.slice(0, 4).map((s) => (
                  <span key={s.id} className="rounded-full bg-rose-50 px-2.5 py-1 capitalize text-rose-700">
                    {s.name}
                  </span>
                ))}
              </div>
      ...
      <section className="container py-12 sm:py-16">
        <div className="rounded-3xl bg-zinc-900 px-8 py-12 text-white shadow-[0_18px_60px_-24px_rgba(16,24,40,0.35)]">
          <div className="flex flex-wrap items-center justify-between gap-4">
            <div>
              <p className="text-sm uppercase text-rose-200">Build your vendor team</p>
              <Heading level={2} className="text-3xl font-semibold text-white">
                From shortlists to booked—one guided path.
              </Heading>
              <Text className="mt-2 text-zinc-200">
          ...
          <div className="mt-10 grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
            {[
              ...
              <div key={item.title} className="rounded-2xl border border-white/10 bg-white/5 p-5 backdrop-blur">
                <p className="text-sm uppercase text-rose-200">{item.title}</p>
                <Text className="mt-2 text-sm text-zinc-100">{item.desc}</Text>
              </div>
            ))}
          </div>
        </div>
      ...
      <section className="container py-12 sm:py-16">
        <div className="flex flex-wrap items-center justify-between gap-4">
          <div>
            <p className="text-sm uppercase text-rose-700">We’ll walk through every part of planning</p>
            <Heading level={2} className="text-3xl font-semibold text-zinc-900">
          ...
          {[
            ...
            <div
              ...
              className={clsx(
                ...
                idx === 0 && 'bg-gradient-to-br from-rose-50 to-white',
                idx === 1 && 'bg-gradient-to-br from-indigo-50 to-white',
                idx === 2 && 'bg-gradient-to-br from-amber-50 to-white'
              )}
            >
              <p className="text-sm uppercase text-rose-700">{item.title}</p>
...
function CTAButton({
  ...
  const base =
    'inline-flex items-center justify-center rounded-full px-4 py-2.5 text-sm font-semibold transition focus:outline-none focus:ring-2 focus:ring-rose-200'
  const styles =
    variant === 'primary'
