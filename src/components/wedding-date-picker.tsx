...
export default function WeddingDatePicker({ className = '' }: { className?: string }) {
  ...
  return (
    <div className={`rounded-xl border border-rose-100 bg-white p-5 shadow-sm ${className}`}>
      <div className="flex flex-col gap-4">
        <div className="space-y-1">
          <p className="text-xs uppercase tracking-wide text-rose-600">Your wedding</p>
          <div className="text-2xl font-semibold text-zinc-900">{coupleNames}</div>
        </div>
        ...
        <div className="grid gap-3 sm:grid-cols-[1.1fr,0.9fr]">
          <div className="rounded-lg border border-zinc-200 bg-white px-3 py-2">
            ...
            <input
              ...
              placeholder="Name"
              className="mt-1 w-full rounded-md border border-zinc-200 px-3 py-2 text-sm focus:border-rose-300 focus:outline-none"
            />
          </div>
          <div className="rounded-lg border border-zinc-200 bg-white px-3 py-2">
        ...
        <div className="rounded-lg border border-zinc-200 bg-white p-3">
          ...
          <div className="mt-2 flex flex-col gap-2 sm:flex-row sm:items-center sm:gap-3">
            <input
              ...
              className="w-full rounded-md border border-zinc-200 px-3 py-2 text-sm focus:border-rose-300 focus:outline-none sm:w-auto"
            />
            {countdown !== null ? (
              <div className="flex flex-wrap items-center gap-3 rounded-md bg-rose-50 px-3 py-2 text-sm font-semibold text-rose-700">
                <div className="flex items-baseline gap-1">
                  <span className="text-xl font-bold leading-none tabular-nums">{countdown.days}</span>
                  <span className="text-[11px] uppercase tracking-wide text-rose-600">days</span>
                </div>
                <div className="flex items-baseline gap-1">
                  <span className="text-xl font-bold leading-none tabular-nums">{countdown.hours}</span>
                  <span className="text-[11px] uppercase tracking-wide text-rose-600">h</span>
                </div>
                <div className="flex items-baseline gap-1">
                  <span className="text-xl font-bold leading-none tabular-nums">{countdown.minutes}</span>
                  <span className="text-[11px] uppercase tracking-wide text-rose-600">m</span>
                </div>
                <div className="flex items-baseline gap-1">
                  <span className="text-xl font-bold leading-none tabular-nums">{countdown.seconds}</span>
                  <span className="text-[11px] uppercase tracking-wide text-rose-600">s</span>
                </div>
